...
const InputComponent = React.forwardRef<HTMLInputElement, InputProps>(({ className, ...props }, ref) => (
  <Input className={cn('w-full pl-20', className)} {...props} ref={ref} />
))
...
const CountrySelect = ({ disabled, value: selectedCountry, options: countryList, onChange }: CountrySelectProps) => {
  ...
  React.useEffect(() => {
    if (!selectedCountry) onChange(t('config.defaults.country').split('-')[1] as any)
  }, [])
    ...
    <Popover>
      <PopoverTrigger asPet>
        <Button
          type='button'
          variant='outline'
          className='flex absolute gap-1 rounded-e-none rounded-s-md px-3 h-[46px] ml-1 mt-1 border-0 focus:z-10'
          disabled={disabled}
        >
...
const FlagComponent = ({ country, countryName }: RPNInput.FlagProps) => {
  ...
  return (
    <span className='bg-foreground/20 flex items-center justify-center h-4 w-6 overflow-hidden rounded-sm'>
